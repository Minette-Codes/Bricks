...
/* (Bastard Operator From Hell) Old nerd humour. */
/* See: https://bofh.bombeck.io/ */

/* REC='{"data":{"id":31,"excuse":"cellular telephone interference"},"meta":{"total":453},"error":null}' */
/* CALL JSON_PARSE REC */
CALL JSON_GET "https://bofh.bombeck.io/v1/excuses/random"
SCR. = ''

EXCUSE_NUM = JSON_VALUE('.data.id')
EXCUSE_TOTAL = JSON_VALUE('.meta.total')
SCR.EXC_NUM = CENTER('Excuse #' EXCUSE_NUM 'of' EXCUSE_TOTAL, 80)
SCR.EXC_TXT = CENTER(JSON_VALUE('.data.excuse'), 80)
IF JSON_IS_STRING('.error') THEN
  SCR.MSG = JSON_VALUE('.error')
